...
void * work(void * ptr)
{
	...
	up = (n-1);
       // printf("\n low = %d\n up = %d \n",low,up);
        ...
        sup(conn->sock, &n , sizeof(int),0);
        sup(conn->sock, arr  , n*sizeof(int),0);
        sup(conn->sock, &low, sizeof(int),0);
        sup(conn->sock, &up, sizeof(int),0);
...
int main(int argc, char ** argv)
{
	...
	//printf("\n final sum = %d",sum);
	if(sup(connect->sock, &sum , sizeof(int),0)<0)
	{
